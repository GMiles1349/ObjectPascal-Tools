    ...
    if aColor.A < 255 then begin
        ...
        Pos := (Z * Self.Width) + Rect.Left;
	      ...
	      for I := Rect.Left to Rect.Right do begin
            ...
            AlphaBlend(PInteger(@aColor), PInteger(@OutColor[Pos]));
	        end;
...
MColor: TPGLColorI;
    ...
    for Z := 0 to DHeight - 1 do begin
      for I := 0 to DWidth - 1 do begin
        ...
        if SPtr[SPos +3] < 255 then begin
        	//PPGLColorI(@DPtr[DPos])^ := Mix(PPGLColorI(@DPtr[DPos]), PPGLColorI(@SPtr[SPos]), SPtr[SPos + 3]);
          AlphaBlend(PInteger(@SPtr[SPos]), PInteger(@DPtr[DPos]));
        end else begin
