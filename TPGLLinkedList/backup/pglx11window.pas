...
uses
	Classes, SysUtils, Linux, UnixType, ctypes, XAtom, XUtil, XLib, X, KeySym,
  UnitUtil;
...
type
  ...
  TPGLKeyboard = class(TPersistent)
    ...
    public
      ...
      constructor Create(aWindow: TPGLWindow);
...
constructor TPGLWindow.Create(aWidth, aHeight: Cardinal; aTitle: String);
var
EventMask: clong;
	begin
    Self.fWidth := aWidth;
...
procedure TPGLWindow.StopClose();
	begin
  	Self.fShouldClose := False;
  end;

procedure TPGLWindow.PollEvents();
var
KeyCode: Integer;
...
procedure TPGLWindow.HandleClose();
	begin
...
procedure TPGLWindow.LockSize(aWidth: Cardinal = 0; aHeight: Cardinal = 0; aLocked: Boolean = false);
var
...
function TPGLWindow.GetImageData(): Pointer;
	begin
  	Exit(Pointer(Self.fImage^.data));
  end;


procedure TPGLWindow.Clear();
var
I: Integer;
PixelCount: Integer;
Pos: Integer;
...
procedure TPGLWindow.SetClearColor(aColor: culong);
	begin
    Self.fClearColor := aColor;
  	XSetWindowBackground(Self.fDisplay, Self.fHandle, aColor);
  end;

procedure TPGLWindow.SetWidth(aWidth: Cardinal);
	begin
  	Self.SetSize(aWidth, Self.fHeight);
  end;

procedure TPGLWindow.SetHeight(aHeight: Cardinal);
	begin
  	Self.SetSize(Self.fWidth, aHeight);
  end;

procedure TPGLWindow.SetSize(aWidth, aHeight: Cardinal);
	begin
    XResizeWindow(Self.fDisplay, Self.fHandle, aWidth, aHeight);
  end;

procedure TPGLWindow.SetSizeable(aSizeable: Boolean = true);
	begin
    Self.fSizeable := aSizeable;
  	Self.LockSize(Self.fWidth, Self.fHeight, not aSizeable);
  end;

procedure TPGLWindow.SetHasTitleBar(aHasTitleBar: Boolean = true);
	begin
...
procedure TPGLWindow.SetOnTryCloseProc(aProc: TPGLProc);
	begin
  	Self.fTryCloseProc := aProc;
  end;

procedure TPGLWindow.SetOnCloseProc(aProc: TPGLProc);
	begin
  	Self.fCloseProc := aProc;
  end;


procedure TPGLWindow.SetOnMoveProc(aProc: TPGLProc);
	begin
  	Self.fMoveProc := aProc;
  end;

procedure TPGLWindow.SetOnResizeProc(aProc: TPGLProc);
	begin
  	Self.fResizeProc := aProc;
  end;

procedure TPGLWindow.SetGotFocusProc(aProc: TPGLProc);
	begin
  	Self.fGotFocusProc := aProc;
  end;

procedure TPGLWindow.SetLostFocusProc(aProc: TPGLProc);
	begin
  	Self.fLostFocusProc := aProc;
  end;

procedure TPGLWindow.UpdateWindowProperties();
var
Atoms: PAtom;
NumProps: cint;
I: Integer;
...
function TPGLWindow.GetWindowPropDesc(aPropAtom: TAtom): TPGLWindowPropDesc; register;
var
AtomName: PChar;
ReturnType: TAtom;
...
procedure TPGLWindow.SetText(aText: string);
	begin
  	XDrawString(Self.fDisplay, Self.fHandle, Self.fGC, 0, 12, PChar(aText), Length(aText));
  end;

procedure TPGLWindow.BlitImage(aImage: Pointer; aDestX, aDesty, aSrcWidth, aSrcHeight: Cardinal);
var
...
constructor TPGLKeyboard.Create(aWindow: TPGLWindow);
	begin
